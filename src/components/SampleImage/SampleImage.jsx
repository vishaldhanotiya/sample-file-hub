...
const SampleImage = () => {
  ...
  useEffect(() => {
    if (imageTabData.length > 0 && imageTabData[0].key) {
      setActiveTab(fileType || imageTabData[0].key);
      fetchFiles(fileType || imageTabData[0].key);
    }
  }, [imageTabData, fileType]);
  ...
  const fetchFiles = async (type) => {
    ...
    try {
      ...
      if (!cacheSnap.empty) {
        const cachedDocs = cacheSnap.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
      ...
      if (!serverSnap.empty) {
        const serverDocs = serverSnap.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
  ...
  return (
    <div className="container mt-4 py-5" style={{ maxWidth: "1200px" }}>
      ...
      <div className="row justify-content-center">
        ...
        <div className="row custom-grid g-3 justify-content-center">
